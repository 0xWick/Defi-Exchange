...
contract Exchange is ERC20 {
    ...
    function addLiquidity(uint256 _amount) public payable returns (uint256) {
        ...
        `Eteher` and `Crypto Dev` tokens because there is no ratio currently
        */
        if (cryptoDevTokenReserve == 0) {
            // Transfer the `cryptoDevToken` from the user's account to the contract
            cryptoDevToken.transferFrom(msg.sender, address(this), _amount);
            // Take the current ethBalance and mint `ethBalance` amount of LP tokens to the user.
            // `liquidity` provided is equal to `ethBlance` because this is the first time user
            ...
            `Ether` and determine according to the ration how many `Crypto Dev` tokens
            ...
            // Ratio should always be maintained so that there are no mahor price impacts when adding liquidity
            ...
            // The amount of LP tokens that would be sent to the user should be proportional to the liquidity
            // of ther added by the user
    ...
    {
        require(_amount > 0, "_amoung should be greater than zero");
        ...
        // The maount of Crypto Dev token that would be sent back to the user is based
        // on a ratio
        // Ration is -> (Crypto Dev sent back to the user/ Current Crypto Dev token reserve)
        // = (amount of LP tokens that user wants to withdraw)/ Total Supply of `LP` tokens
        // Then by some maths -> (Crypto Dev sent back to the user/)
        // = (Current Crypto Dev tokens reserve * amount of LP tokens that user wants to withdraw) / Total supply of `LP` tokens
